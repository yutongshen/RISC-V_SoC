/*-----------------------------------------------------*/
// axi_4to1_mux.sv is generated by gen_axi_mux.sh
//
//                                         2021-12-01
//                                           20:00:02
/*-----------------------------------------------------*/
...
logic [  1: 0] s_awburst  [0:   3];
logic [  9: 0] s_awid     [0:   3];
logic [ 31: 0] s_awaddr   [0:   3];
logic [  2: 0] s_awsize   [0:   3];
logic [  7: 0] s_awlen    [0:   3];
logic [  3: 0] s_wstrb    [0:   3];
logic [  9: 0] s_wid      [0:   3];
logic [ 31: 0] s_wdata    [0:   3];
logic [  9: 0] s_bid      [0:   3];
logic [  1: 0] s_bresp    [0:   3];
logic [ 31: 0] s_araddr   [0:   3];
logic [  1: 0] s_arburst  [0:   3];
logic [  2: 0] s_arsize   [0:   3];
logic [  9: 0] s_arid     [0:   3];
logic [  7: 0] s_arlen    [0:   3];
logic [ 31: 0] s_rdata    [0:   3];
logic [  1: 0] s_rresp    [0:   3];
logic [  9: 0] s_rid      [0:   3];
...
axi_arbitrator_4s u_axi_arbitrator (
    .aclk          ( aclk       ),
    .aresetn       ( aresetn    ),
    .s_arsel       ( s_arsel    ),
    .s_awsel       ( s_awsel    ),
...
always_comb begin
    ...
    for (i = 0; i < 4; i = i + 1) begin
        ...
        m_awid    [  0+:  2] = m_awid[  0+:  2] | ({  2{s_awsel[i]}} & i[0+:2]      );
        m_awid    [  2+: 10] = m_awid[  2+: 10] | ({ 10{s_awsel[i]}} & s_awid    [i]);
    ...
    for (i = 0; i < 4; i = i + 1) begin
        ...
        m_wid     [  0+:  2] = m_wid [  0+:  2] | ({  2{s_wsel[i]}} & i[0+:2]      );
        m_wid     [  2+: 10] = m_wid [  2+: 10] | ({ 10{s_wsel[i]}} & s_wid     [i]);
    ...
    for (i = 0; i < 4; i = i + 1) begin
        ...
        m_arid    [  0+:  2] = m_arid[  0+:  2] | ({  2{s_arsel[i]}} & i[0+:2]      );
        m_arid    [  2+: 10] = m_arid[  2+: 10] | ({ 10{s_arsel[i]}} & s_arid    [i]);
...
always_comb begin
    ...
    for (i = 0; i < 4; i = i + 1) begin
        s_bid     [i] = { 10{bsel == i[0+:  2]}} & m_bid[  2+: 10];
        s_bresp   [i] = {  2{bsel == i[0+:  2]}} & m_bresp;
        s_bvalid  [i] = {  1{bsel == i[0+:  2]}} & m_bvalid;

        s_rdata   [i] = { 32{rsel == i[0+:  2]}} & m_rdata;
        s_rresp   [i] = {  2{rsel == i[0+:  2]}} & m_rresp;
        s_rid     [i] = { 10{rsel == i[0+:  2]}} & m_rid[  2+: 10];
        s_rlast   [i] = {  1{rsel == i[0+:  2]}} & m_rlast;
        s_rvalid  [i] = {  1{rsel == i[0+:  2]}} & m_rvalid;
    end
end

assign bsel = m_bid[  0+:  2];
assign rsel = m_rid[  0+:  2];
...
module axi_arbitrator_4s (
    input                  aclk,
    input                  aresetn,
    output logic  [  3: 0] s_arsel,
    output logic  [  3: 0] s_awsel,
    output logic  [  3: 0] s_wsel,
    input         [  3: 0] s_arvalid,
