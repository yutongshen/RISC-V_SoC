/*-----------------------------------------------------*/
// axi_1to3_dec.sv is generated by ./../../script/gen_axi_dec.sh
//
//                                         2021-11-11
//                                           23:50:32
/*-----------------------------------------------------*/
...
assign awsel[  2] = s_awaddr >= 32'h1000_000 && s_awaddr < 32'h1000_000 + 32'h1000;
assign awsel[  3] = ~|awsel[2:0]; // default slv
...
assign arsel[  2] = s_araddr >= 32'h1000_000 && s_araddr < 32'h1000_000 + 32'h1000;
assign arsel[  3] = ~|arsel[2:0]; // default slv
...
assign m_awvalid = awsel & {4{s_awvalid & ~b_fifo_full}};
assign s_awready = |(awsel & m_awready) & ~b_fifo_full;

assign m_wlast   = wsel & {4{s_wlast }};
assign m_wvalid  = wsel & {4{s_wvalid}};
assign s_wready  = |(wsel  & m_wready );

assign s_bvalid  = |(bsel  & m_bvalid) & ~b_fifo_empty;
assign m_bready  = bsel & {4{s_bready}};

assign m_arvalid = arsel & {4{s_arvalid & ~r_fifo_full}};
...
assign m_rready  = rsel & {4{s_rready}};

axi_dfslv u_axi_dfslv (
    .aclk          ( aclk       ),
    .aresetn       ( aresetn    ),
    .s_awburst     ( m_awburst[3] ),
    .s_awid        ( m_awid[3]  ),
    .s_awaddr      ( m_awaddr[3] ),
    .s_awsize      ( m_awsize[3] ),
    .s_awlen       ( m_awlen[3] ),
    .s_awvalid     ( m_awvalid[3] ),
    .s_awready     ( m_awready[3] ),
    .s_wstrb       ( m_wstrb[3] ),
    .s_wid         ( m_wid[3]   ),
    .s_wdata       ( m_wdata[3] ),
    .s_wlast       ( m_wlast[3] ),
    ...
    .s_bready      ( m_bready[3] ),
    .s_araddr      ( m_araddr[3] ),
    .s_arburst     ( m_arburst[3] ),
    .s_arsize      ( m_arsize[3] ),
    .s_arid        ( m_arid[3]  ),
    .s_arlen       ( m_arlen[3] ),
    .s_arvalid     ( m_arvalid[3] ),
    .s_arready     ( m_arready[3] ),
    .s_rdata       ( m_rdata[3] ),
    .s_rresp       ( m_rresp[3] ),
    .s_rid         ( m_rid[3]   ),
    .s_rlast       ( m_rlast[3] ),
