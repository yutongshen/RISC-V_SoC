...
module axi_slice (
    input                  aclk,
    input                  aresetn,
    input         [ 56: 0] s_awpayload,
    input                  s_awvalid,
    output logic           s_awready,
    input         [ 48: 0] s_wpayload,
    input                  s_wvalid,
    output logic           s_wready,
    output logic  [ 13: 0] s_bpayload,
    output logic           s_bvalid,
    input                  s_bready,
    input         [ 56: 0] s_arpayload,
    input                  s_arvalid,
    output logic           s_arready,
    output logic  [ 46: 0] s_rpayload,
    output logic           s_rvalid,
    input                  s_rready,
    output logic  [ 56: 0] m_awpayload,
    output logic           m_awvalid,
    input                  m_awready,
    output logic  [ 48: 0] m_wpayload,
    output logic           m_wvalid,
    input                  m_wready,
    input         [ 13: 0] m_bpayload,
    input                  m_bvalid,
    output logic           m_bready,
    output logic  [ 56: 0] m_arpayload,
    output logic           m_arvalid,
    input                  m_arready,
    input         [ 46: 0] m_rpayload,
    input                  m_rvalid,
    output logic           m_rready
);

logic [ 56: 0] awpayload_latch [0:   1];
logic [ 48: 0] wpayload_latch [0:   1];
logic [ 13: 0] bpayload_latch [0:   1];
logic [ 56: 0] arpayload_latch [0:   1];
logic [ 46: 0] rpayload_latch [0:   1];
...
always_ff @(posedge aclk or negedge aresetn) begin
    if (~aresetn) begin
        awpayload_latch <= { 57'b0,  57'b0};
        wpayload_latch  <= { 49'b0,  49'b0};
        bpayload_latch  <= { 14'b0,  14'b0};
        arpayload_latch <= { 57'b0,  57'b0};
        rpayload_latch  <= { 47'b0,  47'b0};
    end
    else begin
